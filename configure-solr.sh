...
SOLR_DOWNLOAD="https://ftp.drupal.org/files/projects/search_api_solr-4.x-dev.tar.gz"
...
if [ ! -e "$SOLR_SETUP_COMPLETE_FILE" ]; then
  ...
  sudo su - solr -c "/opt/solr/bin/solr create -c $SOLR_CORE_NAME -d $SOLR_DOWNLOAD_DIR/$SOLR_MODULE_NAME/jump-start/solr$SOLR_VERSION/config-set/"
